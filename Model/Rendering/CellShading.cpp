...
vec3 CellShading::shading(shared_ptr<Scene> scene, HitInfo& info, vec3 lookFrom, std::vector<shared_ptr<Light>> lights, vec3 globalLight){
    ...
    for(std::shared_ptr<Light> l: lights){
        ...
        tempColor = globalLight+intFactor*color;
